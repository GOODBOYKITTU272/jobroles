...
import { ApplicationExpansionView } from '@/components/dashboard/ApplicationExpansionView'
import { CareerStrategySummaryView } from '@/components/dashboard/CareerStrategySummaryView'

const FILTERS = ['Job Volume', 'Salary', 'Growth Outlook', 'AI Exposure', 'Match Score', 'Summary'] as const
...
const LEGEND: Record<Filter, { low: string; high: string }> = {
  ...
  'Summary':        { low: 'Overview',     high: 'Strategy' },
}
const NAV = ['Dashboard', 'Application Expansion']
...
export default function DashboardClient({
  roles, parsedRole, parsedSkills, domain, atsScore, employabilityScore, topScore, education, candidateName
}: DashboardProps) {
  ...
  const [activeNav, setActiveNav] = useState<'Dashboard' | 'Application Expansion'>('Dashboard')
  ...
  async function fetchInsight(role: RoleData, mode: Filter) {
    if (mode === 'Summary') {
      setInsight('Overview of strategic application rules and naming conventions.')
  ...
  const rowHeight = 'minmax(96px, auto)'
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="hidden md:flex items-center gap-0.5">
            {NAV.map(n => (
              <button 
                key={n} 
                onClick={() => setActiveNav(n as any)}
                className={'text-xs px-3 py-1.5 rounded-md transition-all font-medium ' +
                  (activeNav === n ? 'text-white bg-white/10' : 'text-gray-400 hover:text-white hover:bg-white/5')}
              >
                {n}
              </button>
            ))}
          ...
          <>
            ...
            {colorMode !== 'Summary' && (
              <Sidebar 
                ...
                topScore={topScore} 
                onViewSummary={() => setColorMode('Summary')}
              />
            )}
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
              ...
              <div className="flex-1 p-5 overflow-y-auto">
                {colorMode === 'Summary' ? (
                  ...
                  <div className="grid grid-cols-4 gap-2.5" style={{ gridAutoRows: rowHeight }}>
                    {roles.map(r => (
            ...
            {/* RIGHT DETAIL PANEL */}
            {selected && colorMode !== 'Summary' && (
              <RoleDetailsPanel 
                role={selected} 
                education={education} 
                onClose={() => setSelected(roles[0] ?? null)} 
                insight={insight}
                insightLoading={insightLoading}
                insightMetric={colorMode}
                onMetricSelect={setColorMode}
              />
            )}
          </>
        ) : (
          <main className="flex-1 overflow-y-auto bg-[#0d0f1a]">
            <ApplicationExpansionView roles={roles} />
          </main>
        )}
      </div>
    </div>
